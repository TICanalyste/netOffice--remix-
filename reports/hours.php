...
if ($comptListHours != "0") {
    ...
    $block1->labels($labels = array(0 => $strings["organization"], 1 => $strings["project"], 2 => $strings["task"], 3 => $strings["comment"], 4 => $strings["name"], 5 => $strings["date"], 6 => $strings["type"], 7 => $strings["service"], 8 => $strings["hours"]), "true");
    ...
    for ($i = 0;$i < $comptListHours;$i++) {
        ...
        $block1->cellRow(buildLink("../tasks/viewtask.php?id=" . $listHours->tim_task[$i], $listHours->tim_tas_name[$i], LINK_INSIDE));
        $block1->cellRow($listHours->tim_comments[$i]);
