...
$searchProjects = "WHERE (LOWER(pro.name) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchProjects .= " OR LOWER(pro.name) like '%$mots[$z]%' ";
    $z++;
} 
$searchProjects .= " OR LOWER(pro.description) like '%$mots[0]%'";
$y = 1;
while ($y < $nombre_mots) {
    $searchProjects .= " OR LOWER(pro.description) like '%$mots[$y]%' ";
    $y++;
} 

$searchProjects .= " OR LOWER(org.name) like '%$mots[0]%'";
$x = 1;
while ($x < $nombre_mots) {
    $searchProjects .= " OR LOWER(org.name) like '%$mots[$x]%' ";
    $x++;
} 
$searchProjects .= ")";

$searchTasks = "WHERE (LOWER(tas.name) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchTasks .= " OR LOWER(tas.name) like '%$mots[$z]%' ";
    $z++;
} 
$searchTasks .= " OR LOWER(tas.description) like '%$mots[0]%'";
$y = 1;
while ($y < $nombre_mots) {
    $searchTasks .= " OR LOWER(tas.description) like '%$mots[$y]%' ";
    $y++;
} 
$searchTasks .= ")";

$searchMembers = "WHERE (LOWER(mem.login) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchMembers .= " OR LOWER(mem.login) like '%$mots[$z]%' ";
    $z++;
} 
$searchMembers .= " OR LOWER(mem.name) like '%$mots[0]%'";
$y = 1;
while ($y < $nombre_mots) {
    $searchMembers .= " OR LOWER(mem.name) like '%$mots[$y]%' ";
    $y++;
} 

$searchOrganizations = "WHERE (LOWER(org.name) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchOrganizations .= " OR LOWER(org.name) like '%$mots[$z]%' ";
    $z++;
} 
$searchOrganizations .= ")";

$searchTopics = "WHERE LOWER(topic.subject) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchTopics .= " OR LOWER(topic.subject) like '%$mots[$z]%' ";
    $z++;
} 

$searchNotes = "WHERE LOWER(note.subject) like '%$mots[0]%'";
$z = 1;
while ($z < $nombre_mots) {
    $searchNotes .= " OR LOWER(note.subject) like '%$mots[$z]%' ";
    $z++;
} 
