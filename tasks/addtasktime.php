...
if($worker_id=="") $worker_id=$_SESSION['idSession'];

//Get details from existing Task Time if in a "copy" (ID is defined in URL)
if($ttid) {
	$tmpquery = "WHERE tim.id = '$ttid'";
	$taskTimeDetail = new request();
	$taskTimeDetail->openTaskTime($tmpquery);
	$hr=$taskTimeDetail->tim_hours[0];
	$ld=$taskTimeDetail->tim_date[0];
	$comm=$taskTimeDetail->tim_comments[0];
}
...
//if (($teamMember == "false" && $projectsFilter == "true" && (!loggedUserIsAdmin())) || $taskDetail->tas_milestone[0] == "0") {
//    header("Location:../general/permissiondenied.php");
//	echo "add task time denied";
//    exit;
//}
...
if ($_GET['action'] == 'add') {
    ...
    if (empty($msgLabel)) {
        $validated=loggedUserIsAdmin();
        $tasktime_type=TASKTIME_TYPE_TS;
        
        $comm = addSlashes($comm); // resolves bug #768688
        $tmpquery1 = 'INSERT INTO ' . $tableCollab['tasks_time'] . " (project,task,owner,date,hours,comments,created,modified,validated,type,encoder) VALUES ('" . $projectDetail->pro_id[0] . "', '$id','$owner','$ld','$hr','$comm',NOW(),NOW(),$validated,$tasktime_type,".$_SESSION['idSession'].")";
//      echo $tmpquery1;
...

{
	...
	$block1->openForm("../tasks/addtasktime.php?id=$id&amp;project=" . $projectDetail->pro_name[0] . "$popup_addon&amp;action=add#" . $block1->form . "Anchor");
...
//	if(false) {
		...
		for ($i = 0;$i < $comptProjmem;$i++) {
		    ...
		    if ($worker_id == $projmem->tea_mem_id[$i]) {
		...
		echo "<input type='hidden' name='owner' value='".$_SESSION['idSession']."'>".$_SESSION['nameSession'];
	}
	if ($ld == '') {
	    $ld = $date;
	}
...
if($window=="popup") {
	...
	<tr class='odd'>
	  ...
	  <td><input size='20' value='$hr' style='width: 150px;' name='hr' maxlength='6' type='text' onChange='timeCheck(this);'></td>
	</tr>
	<tr class='odd'>
	...
	<tr class='odd'>
	  ...
	  <td><input type='SUBMIT' value='" . $strings["save"] . "'><input type='button' name='cancel' value='" . $strings['cancel'] . "' onClick='$cancel_action'></td>
	</tr>";
...
// This will display time log detail for the current task (only if not in a popup)
if($window!="popup") {
	$block2 = new block();
